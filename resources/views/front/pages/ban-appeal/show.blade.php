...
@section('body')
    ...
    <div class="max-w-screen-xl m-auto px-3 md:px-9 mb-12">
            ...
            @switch($banAppeal->status)
                @case(BanAppealStatus::ACCEPTED_UNBAN)
                    <div class="rounded-lg bg-green-100 p-6 mt-6">
                        <h3 class="text-xl font-bold mb-3">Appeal Accepted</h3>
                        Your ban appeal has been accepted.<br />
                        <strong>You must read the below response from staff, as it may contain important information to prevent you being banned in future.</strong>
                    </div>
                    @break
        ...
        @if($banAppeal->status->isDecided())
            <div class="rounded-lg bg-gray-50 p-6 mt-2">
                <div class="flex flex-row gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
