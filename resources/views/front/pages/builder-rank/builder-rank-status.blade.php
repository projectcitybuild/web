            ...
            <ol class="flex items-center w-full text-sm font-medium text-center text-gray-500 sm:text-base mt-6">
                ...
                @if ($application->isReviewed())
                    ...
                    <li class="flex items-center text-blue-600">
                        <span
                            class="flex items-center after:content-['/'] sm:after:hidden after:mx-2 after:text-gray-200">
                            <svg class="w-3.5 h-3.5 sm:w-6 sm:h-6 me-2.5" aria-hidden="true"
            ...
            <ol class="relative border-s border-gray-200">
                ...
                @if ($application->status === ApplicationStatus::APPROVED)
                    <li class="mb-10 ms-6">
                        ...
                        <div
                            class="items-center justify-between p-4 bg-white border border-gray-200 rounded-lg sm:flex">
                            <time
                                class="mb-1 text-xs font-normal text-gray-400 sm:order-last sm:mb-0">{{ $application->closed_at }}</time>
                            <div class="text-sm font-normal text-gray-500 lex">Application was <span
                                    class="font-semibold text-green-500">Approved</span> and closed
                            </div>
                        </div>
                    </li>
                @endif
                        ...
                        <div class="p-4 bg-white border border-gray-200 rounded-lg">
                            <div class="items-center justify-between mb-3 sm:flex">
                                <time
                                    class="mb-1 text-xs font-normal text-gray-400 sm:order-last sm:mb-0">{{ $application->closed_at }}</time>
                                <div class="text-sm font-normal text-gray-500 lex">Application was <span
                                        class="font-semibold text-red-500">Unsuccessful</span> with the following
                                    comment
                                </div>
                            </div>
                            <div
                ...
                <li class="mb-10 ms-6">
                    ...
                    <div class="items-center justify-between p-4 bg-white border border-gray-200 rounded-lg sm:flex">
                        <time
                            class="mb-1 text-xs font-normal text-gray-400 sm:order-last sm:mb-0">{{ $application->created_at }}</time>
                        <div class="text-sm font-normal text-gray-500 lex">Application set to <span
                                class="font-semibold text-black">In Review</span></div>
                    </div>
                </li>
                ...
                <li class="mb-10 ms-6">
                    <span
                        class="absolute flex items-center justify-center w-6 h-6 rounded-full -start-3 ring-8 ring-white">
                        <img
                            src="https://minotar.net/avatar//{{ $application->account->minecraftAccount?->first()?->alias }}/16"
                            class="rounded-md h-6">
                    </span>
                    <div class="items-center justify-between p-4 bg-white border border-gray-200 rounded-lg sm:flex">
                        <time
                            class="mb-1 text-xs font-normal text-gray-400 sm:order-last sm:mb-0">{{ $application->created_at }}</time>
                        <div class="text-sm font-normal text-gray-500 lex">
                            <strong>{{ $application->minecraft_alias }}</strong> submitted an application (as
                            <strong>{{ $application->current_builder_rank }}</strong>)
                        </div>
                    </div>
                </li>
            </ol>
