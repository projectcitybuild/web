<div class="row mb-3">
    ...
    <div class="col-sm-4">
        <select name="activity_type" id="activity_type" data-pcb-navigation-select data-pcb-search-select>
            <option value="{{ route('manage.activity.index') }}" placeholder>all</option>
            @foreach($allActions as $action)
                <option
                    value="{{ route('manage.activity.index', array_merge($activeFilters, $action->only('subject_type', 'description'))) }}"
